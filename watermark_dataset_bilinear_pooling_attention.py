...
class L_DATASET_(BASE_DATASET):
    ...
    def generate(self, base_dataset):
        ...
        while True:
            ...
            # --- Modified for Attention Mechanism below ---
            ...
            attention_scores = attention_scores / (c ** 0.5)  # scale by sqrt(d_k)

            attention_weights = torch.softmax(attention_scores, dim=-1)  # (batch, h*w, h*w)

            # Attend image2 features based on image1
            attended_features = torch.bmm(attention_weights, image2_flat.transpose(1, 2))  # (batch, h*w, channels)
            ...
            # Add small attention perturbation
            images = (1 - lambda_) * image1 + lambda_ * image2 + 0.1 * attended_features
            # --- End of Attention Mechanism modification ---
